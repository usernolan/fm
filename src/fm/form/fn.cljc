...
(defmethod form/form ::combined-argxs
  ...
  ;; NOTE: in nominal contexts, args and ret data is filtered using
  ;; `select-keys` for conformance so that only keys specified in the `:fm/args`
  ;; and `:fm/ret` spec vectors are validated. this avoids confusion from
  ;; anomalies generated by irrelevant input or output keys. see the `Maps`
  ;; section of https://clojure.org/about/spec#_features for more info on the
  ;; feature of spec that prompts this workaround
  ;;
  ;; NOTE: specifying `:fm/args` or `:fm/ret` directly in the function metadata
  ;; will not trigger the `select-keys` filtering, and will cause all invalid
  ;; qualified keys to fail fast, regardless of their relevance to the function
  ;; being specified
(defmethod form/form [::args ::s/conformed]
  [ctx _]
  (let [spec    (form/form ctx [::form/binding ::form/fn :fm/args])
        ...
        data    (if (= context :fm.context/nominal)
                  (let [ks (first (get-in ctx [::metadata :fm/args index]))]
                    `(select-keys ~args ~ks))
                  args)]
    `(s/conform ~spec ~data)))

(defmethod form/form [::combined-argxs ::s/conformed]
  [ctx _]
  (let [spec     (form/form ctx [::form/binding ::form/fn :fm/args])
        args     (form/form ctx [::form/binding ::form/fn ::combined-argxs])
        contexts (map :fm/args (get ctx ::signature-contexts))
        data     (if (every? #{:fm.context/nominal} contexts)
                   (let [ks (into (hash-set)
                                  (comp (map first) cat)
                                  (get-in ctx [::metadata :fm/args]))]
                     `(select-keys ~args ~ks))
                   args)]
    `(s/conform ~spec ~data)))
...
(defmethod form/form [::ret ::s/conformed]
  ...
  (let [spec    (form/form ctx [::form/binding ::form/fn :fm/ret])
        ret     (form/form ctx [::form/binding ::form/fn ::ret])
        index   (signature-index ctx)
        context (get-in ctx [::signature-contexts index :fm/ret])
        data    (if (= context :fm.context/nominal)
                  (let [ks (first (get-in ctx [::metadata :fm/ret index]))]
                    `(select-keys ~ret ~ks))
                  ret)]
    `(s/conform ~spec ~data)))
